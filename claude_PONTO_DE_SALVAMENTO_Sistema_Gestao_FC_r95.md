...
## 5. Análise de Inventários (r64–r95)
...
**(r95) BUG corrigido — "Selecionar todas" parecia não fazer nada.** Causa: `popularDropdownFiltro()` (função que já existia, redesenha os checkboxes) tem um efeito colateral não documentado — toda vez que é chamada, ela sobrescreve sozinha `filtrosGeralEstado[prefixo]` internamente. A Fase 4 da cascata (`recalcularFiltrosGeralCascata`) chamava essa função e, LOGO DEPOIS, relia em `filtrosGeralEstado[prefixo]` pra saber quais caixinhas marcar — só que nesse ponto o valor já tinha sido sobrescrito pela própria função que acabou de rodar, não era mais o valor correto calculado na Fase 3. Resultado: ao desmarcar "Selecionar todas", as caixinhas voltavam a ficar marcadas na mesma fração de segundo — parecia que nada tinha acontecido. Corrigido guardando o resultado da Fase 3 numa variável local (`novoEstado`), usada pra marcar as caixinhas na Fase 4 em vez de reler `filtrosGeralEstado` — e restaurando `filtrosGeralEstado` pro valor certo ao final, senão a PRÓXIMA chamada partiria do estado errado de novo.
...
**Versão exibida na tela de login** — `VERSAO_ESPERADA`/`VERSAO_SCRIPT`/`VERSAO` sincronizados em `2026-09-03-r95` / `r95` (pendente publicar — ver seção 11).
...
- **(r95) Regra de efeito colateral escondido:** antes de reler uma variável de estado compartilhado (ex: `filtrosGeralEstado[x]`) logo depois de chamar uma função, checar se essa função já não escreve nessa mesma variável por dentro dela mesma, como efeito colateral — `popularDropdownFiltro` faz isso e não está documentado no nome/assinatura dela. Nesses casos, guardar o valor certo numa variável local ANTES da chamada e usar a local depois, nunca reler a variável compartilhada logo em seguida.
...
r95: Correção de bug — "Selecionar todas" nos filtros de Despesas parecia não fazer nada (nada era desmarcado). Causa: efeito colateral não documentado de `popularDropdownFiltro` (sobrescreve `filtrosGeralEstado` sozinha ao redesenhar os checkboxes), combinado com a Fase 4 da cascata relendo essa mesma variável logo depois — lia o valor recém-sobrescrito, não o calculado corretamente na Fase 3. Corrigido usando variável local pro valor certo, restaurando `filtrosGeralEstado` ao final.
...
- [ ] **(r95)** Publicar os 9 arquivos: GitHub Pages (frontend) + Nova Versão no Apps Script (Implantar → Gerenciar implantações → lápis → Nova versão → Implantar) — os três lados (`VERSAO_ESPERADA`, `VERSAO_SCRIPT`, `VERSAO` do auditoria.html) precisam ficar em r95 ao mesmo tempo.
- [ ] **(r95)** Usuário precisa atualizar manualmente os 9 arquivos em `codigo/<nome-do-arquivo>` no Project Knowledge (Claude não escreve lá diretamente) — inclusive este checkpoint.
- [ ] **(r95) CRÍTICO:** testar em produção "Selecionar todas" nos 4 filtros de Despesas — desmarcar tudo, confirmar que fica desmarcado, marcar itens específicos, confirmar que a cascata e a tabela reagem certo.
