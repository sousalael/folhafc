...
// r137: helper de diagnóstico de desempenho — mede quanto cada etapa leva,
// sem mudar nenhum comportamento. Só escreve no Logger (Execuções do Apps
// Script); não aparece pro usuário nem afeta o resultado de nada.
function _perfMarca(t0, rotulo) {
  var agora = new Date().getTime();
  Logger.log('[PERF] ' + rotulo + ': ' + (agora - t0) + 'ms');
  return agora;
}
...
function gerarRelatorioAuditoria(dados, cpf) {
  try {
    ...
    var aba = getOuCriarAbaAuditoria();
    var todas = aba.getDataRange().getValues();
    _t0 = _perfMarca(_t0, 'relatorio: leitura da aba Auditoria_Operacao inteira (' + todas.length + ' linhas)');
    var row = null;
    ...
    var pasta = DriveApp.getFolderById(pastaId);
    _t0 = _perfMarca(_t0, 'relatorio: localizar/criar pasta no Drive');
    ...
    _t0 = _perfMarca(_t0, 'relatorio: limpeza de arquivos antigos (getFilesByName x3 + trash)');
    ...
    _t0 = _perfMarca(_t0, 'relatorio: criar arquivo HTML + compartilhar');
    var pdfBlob = arquivo.getAs('application/pdf');
    _t0 = _perfMarca(_t0, 'relatorio: converter HTML -> PDF (getAs)');
    ...
    _t0 = _perfMarca(_t0, 'relatorio: criar arquivo PDF + compartilhar');
...
function prepararApresentacaoAuditoria(dados, cpf) {
  try {
    ...
    var aba = getOuCriarAbaAuditoria();
    var todas = aba.getDataRange().getValues();
    _t0 = _perfMarca(_t0, 'apresentacao-prep: leitura da aba Auditoria_Operacao inteira (' + todas.length + ' linhas)');
    var row = null;
    for (var i = 1; i < todas.length; i++) { if (todas[i][0] === auditoriaId) { row = todas[i]; break; } }
    if (!row) return { ok: false, erro: 'Auditoria não encontrada' };
    ...
    _t0 = _perfMarca(_t0, 'apresentacao-prep: chamada a IA (Claude) + parse do texto');
    ...
    _t0 = _perfMarca(_t0, 'apresentacao-prep: buscar bytes das fotos no Drive (até 60 fotos)');
    ...
    Logger.log('[PERF] apresentacao-prep: fotos incluídas = ' + fotosIncluidas);
...
function salvarApresentacaoAuditoria(dados, cpf) {
  try {
    ...
    var aba = getOuCriarAbaAuditoria();
    var todas = aba.getDataRange().getValues();
    _t0 = _perfMarca(_t0, 'apresentacao-salvar: leitura da aba Auditoria_Operacao inteira (' + todas.length + ' linhas)');
    var row = null;
    for (var i = 1; i < todas.length; i++) { if (todas[i][0] === auditoriaId) { row = todas[i]; break; } }
    if (!row) return { ok: false, erro: 'Auditoria não encontrada' };
    ...
    variantes.forEach(function(v){
      ...
      _t0 = _perfMarca(_t0, 'apresentacao-salvar: limpar versões antigas (' + v.chave + ')');
      ...
      _t0 = _perfMarca(_t0, 'apresentacao-salvar: salvar .pptx no Drive + compartilhar (' + v.chave + ', ' + bytes.length + ' bytes)');
      ...
      try {
        ...
        _t0 = _perfMarca(_t0, 'apresentacao-salvar: converter .pptx -> PDF (' + v.chave + ')');
        ...
        _t0 = _perfMarca(_t0, 'apresentacao-salvar: salvar .pdf no Drive + compartilhar (' + v.chave + ')');
...
function converterPptxParaPdfReal(pptxBlob, nomeBase) {
  ...
  _t0 = _perfMarca(_t0, '  converterPptxParaPdfReal: upload + conversão automática para Slides');
  var pdfBlob;
  try {
    pdfBlob = DriveApp.getFileById(arquivoConvertido.id).getAs('application/pdf');
    _t0 = _perfMarca(_t0, '  converterPptxParaPdfReal: exportar Slides -> PDF');
  } finally {
    try { DriveApp.getFileById(arquivoConvertido.id).setTrashed(true); } catch(e) {}
    _t0 = _perfMarca(_t0, '  converterPptxParaPdfReal: apagar cópia intermediária em Slides');
  }
  return pdfBlob;
}
